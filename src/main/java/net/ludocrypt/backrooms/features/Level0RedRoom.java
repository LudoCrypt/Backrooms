...
public class Level0RedRoom extends Feature<DefaultFeatureConfig> {
	public Level0RedRoom(Codec<DefaultFeatureConfig> configFactory) {
		super(configFactory);
	}
	...
	public boolean generate(StructureWorldAccess world, ChunkGenerator chunkGenerator, Random rand, BlockPos position,
		...
		if (layoutInt == 100) {
			RoomArchives.generateSlice(world, mutableBlockPos.move(Direction.EAST), RoomArchives.One, rand,
					Backrooms.RED_WALLPAPER, Backrooms.RED_TORN_WALLPAPER, STAIR, Backrooms.CARPET, MOLDY_CARPET, true);
