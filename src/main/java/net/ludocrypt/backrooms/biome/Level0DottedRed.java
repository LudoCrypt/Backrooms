...
public class Level0DottedRed extends Biome {

	protected static final BlockState WALL = Backrooms.WALL.getDefaultState();
	public static final Decorator<NopeDecoratorConfig> PLACER = new Level0RoomDecorator(
			NopeDecoratorConfig.field_24891);

	public Level0DottedRed() {
		...
		super(new Biome.Settings()
				...
				.temperature(1.0F).downfall(0F).effects(new BiomeEffects.Builder().waterColor(14402413)
						.waterFogColor(14402413).fogColor(14402413).music(MusicType.method_27283(BackroomsSoundEvents.LEVEL0WEIGHTEDMUSIC)).build()));

		this.addFeature(GenerationStep.Feature.LOCAL_MODIFICATIONS, LevelsFeatureInit.LEVEL0DOTTEDREDROOM
				.configure(FeatureConfig.DEFAULT).createDecoratedFeature(PLACER.configure(DecoratorConfig.DEFAULT)));
	}
