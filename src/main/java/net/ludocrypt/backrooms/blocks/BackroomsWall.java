...
public class BackroomsWall extends Block implements Waterloggable {
	   ...
	   public BackroomsWall(AbstractBlock.Settings settings) {
	      super(settings);
	   ...
	   private Map<BlockState, VoxelShape> getShapeMap(float f, float g, float h, float i, float j, float k) {
	      ...
	      while(var21.hasNext()) {
	         ...
	         Iterator<WallShape> var23 = EAST_SHAPE.getValues().iterator();

	         while(var23.hasNext()) {
	            WallShape wallShape = (WallShape)var23.next();
	            Iterator<WallShape> var25 = NORTH_SHAPE.getValues().iterator();

	            while(var25.hasNext()) {
	               WallShape wallShape2 = (WallShape)var25.next();
	               Iterator<WallShape> var27 = WEST_SHAPE.getValues().iterator();

	               while(var27.hasNext()) {
	                  WallShape wallShape3 = (WallShape)var27.next();
	                  Iterator<WallShape> var29 = SOUTH_SHAPE.getValues().iterator();
	   ...
	   private boolean method_27092(BlockState blockState, BlockState blockState2, VoxelShape voxelShape) {
	      boolean bl = blockState2.getBlock() instanceof BackroomsWall && (Boolean)blockState2.get(UP);
	      if (bl) {
	         return true;
	      } else {
	   ...
	   static {
	      ...
	      field_22163 = Block.createCuboidShape(7.0D, 0.0D, 7.0D, 9.0D, 16.0D, 9.0D);
	      field_22164 = Block.createCuboidShape(7.0D, 0.0D, 0.0D, 9.0D, 16.0D, 9.0D);
	      field_22165 = Block.createCuboidShape(7.0D, 0.0D, 7.0D, 9.0D, 16.0D, 16.0D);
	      field_22166 = Block.createCuboidShape(0.0D, 0.0D, 7.0D, 9.0D, 16.0D, 9.0D);
	      field_22167 = Block.createCuboidShape(7.0D, 0.0D, 7.0D, 16.0D, 16.0D, 9.0D);
	   }
	}