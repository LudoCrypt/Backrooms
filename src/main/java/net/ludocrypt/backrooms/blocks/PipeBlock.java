...
public class PipeBlock extends Block implements Waterloggable {
	...
	public PipeBlock(Settings settings) {
		super(settings);
		setDefaultState(getStateManager().getDefaultState().with(UP, false).with(DOWN, false).with(EAST, false).with(WEST, false).with(NORTH, false).with(SOUTH, false).with(WATERLOGGED, false));
	}
	...
	public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
		...
		if (random.nextInt(2) == 0) {
			world.addParticle(ParticleTypes.MYCELIUM, (double) pos.getX() + (double) random.nextFloat(), (double) pos.getY() + (double) random.nextFloat(), (double) pos.getZ() + (double) random.nextFloat(), 1.0D, -1.0D, 1.0D);
		}
		if (random.nextInt(10) == 0) {
			world.addParticle(ParticleTypes.DRIPPING_WATER, (double) pos.getX() + (random.nextDouble() / 4) + 0.5, (double) pos.getY() + 0.18, (double) pos.getZ() + (random.nextDouble() / 4) + 0.5, 0.0D, 0.0D, 0.0D);
		}
	}
	...
	public BlockState withConnectionProperties(BlockView view, BlockPos pos) {
		...
		return (BlockState) ((BlockState) ((BlockState) ((BlockState) ((BlockState) ((BlockState) this.getDefaultState().with(DOWN, block == BackroomsBlocks.PIPE)).with(UP, block2 == BackroomsBlocks.PIPE)).with(NORTH, block3 == BackroomsBlocks.PIPE)).with(EAST, block4 == BackroomsBlocks.PIPE)).with(SOUTH, block5 == BackroomsBlocks.PIPE)).with(WEST, block6 == BackroomsBlocks.PIPE);
	}
	...
	@Override
	public BlockState getStateForNeighborUpdate(BlockState state, Direction facing, BlockState neighborState, WorldAccess world, BlockPos pos, BlockPos posFrom) {
		Block block = neighborState.getBlock();
		boolean bl = block == BackroomsBlocks.PIPE;
